...
const fmtReadiness=n=>Number(n||0).toLocaleString();
...
router.get('/launch-readiness',async(_req,res,next)=>{
  try{
    ...
    const [[migration]]=await pool.query("SELECT COUNT(*) total FROM schema_migrations WHERE filename='014_launch_readiness.sql'");
    add('migrations','Latest migration applied',Number(migration?.total||0)>0?'PASS':'BLOCKER',
      Number(migration?.total||0)>0?'014_launch_readiness.sql applied':'Latest migration missing','DATABASE');
    ...
    add('cities','Somalia-only launch cities',cityOk?'PASS':'BLOCKER',
      fmtReadiness(Number(cities?.total||0))+' active · '+fmtReadiness(Number(cities?.non_somalia||0))+' non-Somalia','COMPETITION');
    ...
    add('goal_complete','Permanent Goal numbers',Number(goals?.missing||0)===0?'PASS':'BLOCKER',
      Number(goals?.missing||0)===0?fmtReadiness(Number(goals?.verified||0))+' verified memberships complete':fmtReadiness(Number(goals?.missing||0))+' verified memberships missing Goal numbers','INTEGRITY');
    ...
    add('goal_unique','Goal-number uniqueness',Number(duplicates?.duplicate_groups||0)===0?'PASS':'BLOCKER',
      Number(duplicates?.duplicate_groups||0)===0?'No duplicate Goal numbers':fmtReadiness(Number(duplicates.duplicate_groups))+' duplicate groups','INTEGRITY');
    ...
    add('goal_sequence','Goal sequence health',Number(sequence?.bad||0)===0?'PASS':'BLOCKER',
      Number(sequence?.bad||0)===0?'All city Goal sequences are ahead':'Bad sequences: '+fmtReadiness(Number(sequence.bad)),'INTEGRITY');
    ...
    add('fixture_health','Fixture lifecycle health',fixturesOk?'PASS':'BLOCKER',
      ...
      'missing regulation='+fmtReadiness(Number(fixtureHealth?.missing_regulation||0))+
      ' · early lobby='+fmtReadiness(Number(fixtureHealth?.early_lobby||0))+
      ' · early live='+fmtReadiness(Number(fixtureHealth?.early_live||0)),'MATCHES');
    ...
    add('admin_account','Administrator account',Number(admins?.total||0)>0?'PASS':'BLOCKER',
      Number(admins?.total||0)>0?fmtReadiness(Number(admins.total))+' admin credential record(s)':'Create the first administrator at /admin','OPERATIONS');
    ...
    add('tournament','Tournament structure',stageCount>0?'PASS':'WARNING',
      stageCount>0?fmtReadiness(stageCount)+' stage(s) configured':'Not required for qualification launch; configure before Cup stage begins','COMPETITION');
    ...
    for(const [type,label] of evidenceChecks){
    ...
    res.json({
      overall,blockers,warnings,checks,
